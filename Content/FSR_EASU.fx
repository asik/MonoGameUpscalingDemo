...
float4 Pass1(PixelInput input) : SV_TARGET
{
    ...
    uint2 inputSize = uint2(inputWidth, inputHeight);
    float2 inputPt = float2(inputPtX, inputPtY);
    uint2 outputSize = uint2(outputSizeX, outputSizeY);

    // Calculate output position based on texture coordinates
    ...
    // Perform the calculation and output the color
    return float4(FsrEasuF(gxy, con0, con1, con2, con3), 1);
}
